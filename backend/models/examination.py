...
class ExaminationModel(db.Model):
    ...
    def json_with_info(self):
        appointment_data = self.appointment.json() if self.appointment else {}
        return {
            "_id": self.id,
            "diagnosis": self.diagnosis,
            "prescription": self.prescription,
            ...
            "doctor_name": f"{self.appointment.doctor.first_name} {self.appointment.doctor.last_name}" if self.appointment and self.appointment.doctor else "N/A",
            "doctor_specialization": self.appointment.doctor.specialization if self.appointment and self.appointment.doctor else "N/A",
            "patient_name": f"{self.appointment.patient.first_name} {self.appointment.patient.last_name}" if self.appointment and self.appointment.patient else "N/A",
            "appointment_date": self.appointment.date.strftime("%Y-%m-%d") if self.appointment else "N/A",
        }
